...
void draw_tracked_locations(IplImage * frame, template_matching_based_tracker * tracker)
{
  for(int i = 0; i < tracker->nx * tracker->ny; i++) {
    ...
    cvCircle(frame, cvPoint(x1, y1), 2, cvScalar(255, 255, 255), 1);
  }
}
...
void draw_detected_keypoints(IplImage * frame, planar_pattern_detector * detector)
{
  for(int i = 0; i < detector->number_of_detected_points; i++)
    cvCircle(frame,cvPoint(detector->detected_points[i].fr_u(),detector->detected_points[i].fr_v()),
	     2 * (1 << int(detector->detected_points[i].scale)),
	     cvScalar(100), 1);
}
...
int main()
{
  ...
  */
  ...
  //number_of_generated_images_to_find_stable_points, 5000 int number_of_samples_for_refinement 10000
