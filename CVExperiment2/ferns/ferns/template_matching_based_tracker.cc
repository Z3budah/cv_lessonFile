...
{
  int max = -1;
  //cout << xc << " " << yc << " " << w << " " << h <<" "<<xm<<" "<<ym<< endl ;
  for(int v = yc - h / 2; v <= yc + h / 2; v++) {
	if (v >= G->height - 1||v<0) continue;
...
void template_matching_based_tracker::find_2d_points(IplImage * image, int bx, int by)
{
  ...
  for (int j = 0; j < ny; j++)
  {
	  for (int i = 0; i < nx; i++)
	  {
		  int xc = int(u0[0] + bx + i * stepx + 0.5);
		  int yc = int(u0[1] + by + j * stepy + 0.5);
		  int w = int(stepx);
		  int h = int(stepy);
		  //int w = (int(stepx) > xc) ? (int(stepx) % xc): int(stepx);
		  //int h = (int(stepy) > yc) ? (int(stepy) % yc) : int(stepy);

		  get_local_maximum(gradient,xc, yc,w, h,
			  m[2 * (j * nx + i)],
			  m[2 * (j * nx + i) + 1]);
	  }
  }
...
void template_matching_based_tracker::compute_As_matrices(IplImage * image, int max_motion, int Ns)
{
  ...
  for(int level = 0; level < number_of_levels; level++) {
	...
	if(y1>image->height)cout << y1 << " ";
		if (y1>=0&&y1<image->height)  i1[i] = mcvRow(image, y1, unsigned char)[x1];
					    ...
					    int Ns)
{
  ...
  //neighbordhood for local maxima selection
